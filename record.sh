...
NAME="$(basename "$OUT")"
SESSION=$(curl -sS -L -G "$DRIVE_UPLOAD_URL" --data-urlencode "name=$NAME" --data-urlencode "size=$SIZE")
case "$SESSION" in https://*) ;; *) echo "Upload link request failed: $SESSION"; exit 1;; esac
curl -sS --fail -X PUT -H "Content-Type: audio/mp4" --upload-file "$OUT" "$SESSION" -o /dev/null
